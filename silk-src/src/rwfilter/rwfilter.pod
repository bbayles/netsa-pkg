...
When B<rwfilter> is parsing the name of a file, it converts the
...
The B<--class> switch is used to specify a group of data files to
process.  Only a single class may be selected with the B<--class>
switch; for multiple classes, use the B<--flowtypes> switch.  The
argument may be "@I<PATH>" which causes B<rwfilter> to open the file
I<PATH> and read the class name from it; see L<Read Selection Argument
Values from a File> for details.  Classes are defined in the
B<silk.conf(5)> site configuration file.  If neither the B<--class>
nor B<--flowtypes> option is given, the default-class as specified in
F<silk.conf> is used.  To see the available classes and the default
class, either examine the output from B<rwfilter --help> or invoke
B<rwsiteinfo(1)> with the switch B<--fields=class,default-class>.
...
comma-separated list of types names and "@I<PATH>" directives, where
@I<PATH> tells B<rwfilter> to read type names from the file I<PATH>;
...
B<--help> output of B<rwfilter> or run B<rwsiteinfo> with
...
include "@I<PATH>" which causes B<rwfilter> to open the file I<PATH>
...
Values from a File>, @I<PATH> tells B<rwfilter> to read the names of
...
which is a one or more of the following separated by commas: an IPv4
address (C<10.1.2.3>), an IPv6 address (C<2001:db8::10.1.2.3>), an
unsigned 32-bit integer representing an IPv4 address (C<167838211>),
or any of those with a CIDR block designation (C<192.168.0.0/16>,
C<2001:db8::/32>, C<167772160/8>).
...
C<C>=CWR.  As of SiLK 3.20.0, the symbol C<-> is accepted and
represents all TCP flags (C<FSRPAUEC>).
...
specified I<TCP_FLAGS> was set (high), where I<TCP_FLAGS> contains the
letters C<F>,C<S>,C<R>,C<P>,C<A>,C<U>,C<E>,C<C>.  For example,
B<--tcp-flags=ASF> passes records where ACK is set, or SYN is set, or
FIN is set (this is equivalent to B<--flags-all=A/A,S/S,F/F>).

=item B<--ack-flag>={B<0>|B<1>}

Pass the record when either the argument is 1 and the ACK flag is set
(high) or the argument is 0 and the ACK flag is unset (low).

=item B<--cwr-flag>={B<0>|B<1>}

Pass the record when either the argument is 1 and the CWR flag is set
(high) or the argument is 0 and the CWR flag is unset (low).

=item B<--ece-flag>={B<0>|B<1>}

Pass the record when either the argument is 1 and the ECE flag is set
(high) or the argument is 0 and the ECE flag is unset (low).

=item B<--fin-flag>={B<0>|B<1>}

Pass the record when either the argument is 1 and the FIN flag is set
(high) or the argument is 0 and the FIN flag is unset (low).

=item B<--psh-flag>={B<0>|B<1>}

Pass the record when either the argument is 1 and the PSH flag is set
(high) or the argument is 0 and the PSH flag is unset (low).

=item B<--rst-flag>={B<0>|B<1>}

Pass the record when either the argument is 1 and the RST flag is set
(high) or the argument is 0 and the RST flag is unset (low).

=item B<--syn-flag>={B<0>|B<1>}

Pass the record when either the argument is 1 and the SYN flag is set
(high) or the argument is 0 and the SYN flag is unset (low).

=item B<--urg-flag>={B<0>|B<1>}

Pass the record when either the argument is 1 and the URG flag is set
(high) or the argument is 0 and the URG flag is unset (low).
...
I<MASK_ATTRIBUTES>.  As of SiLK 3.20.0, the symbol C<-> is accepted
and represents all attributes (C<STCF>).  B<rwfilter> passes the
record if, for any pair of attributes in the list, the attributes
listed in I<HIGH_ATTRIBUTES> are set and the remaining attributes in
I<MASK_ATTRIBUTES> are not-set.  The valid I<attributes> are:
...
The following four switches are normally L<file selection
switches|/Selection Switches> that select which files B<rwfilter>
reads within the data repository.  However, when B<rwfilter> gets
input without querying the data repository (that is, from files listed
on the command line, from files specified by B<--xargs>, or from the
B<--input-pipe>), these switches become partitioning switches and
determine whether a record is written to the pass-destination or
fail-destination.
...
Pass the record if its class is I<CLASS> or the class named in I<PATH>
and its type is listed in the B<--type> switch, or its type is in the
default type list for I<CLASS> when B<--type> is not specified.
Examine the output of B<rwfilter --help> to see the list of available
classes, available types, and their default values, or use
B<rwsiteinfo --fields=class,type,mark-defaults>.

=item B<--flowtypes>=I<CLASS>/I<TYPE>[,I<CLASS>/I<TYPE>,@I<PATH> ...]

Pass the record its if class/type value is one of those listed or read
from the file I<PATH>.  The keyword C<all> may be used for the
I<CLASS> and/or I<TYPE> to select all classes and/or types.  This
switch cannot be used when either B<--class> or B<--type> is used.
Use B<rwfilter --help> or B<rwsiteinfo --fields=class,type> to see the
...
ranges of sensor IDs, sensor group names, and "@I<PATH>" directives.
Use the B<rwsiteinfo(1)> command to see the list of sensors.  Support
for sensor group names was added in SiLK 3.21.0.

=item B<--type>={C<all> | I<TYPE>[,I<TYPE>,@I<PATH> ...]}

Pass the record if its type is one of those listed or read from the
file I<PATH> and its class is specified by B<--class>, or its class is
the default class when the B<--class> switch is not specified.  Use
B<rwfilter --help> to see the list of available classes, available
types, and their defaults, or use B<rwsiteinfo
--fields=class,type,mark-defaults>.
...
$SiLK: rwfilter.pod 3b66f16540fa 2023-06-02 19:00:07Z mthomas $
