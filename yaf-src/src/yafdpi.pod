##  yafdpi manual page   -*- mode: text; fill-column: 78 -*-
##
##  src/yafdpi.pod.  Generated from yafdpi.pod.in by configure.
##
...
label E<lt>NE<gt> element E<lt>N2E<gt> E<lt>element-ruleE<gt>

where E<lt>NE<gt> is the application label (usually the well-known port) found in the yafApplabelRules.conf file (an unsigned 16-bit decimal integer in the range 0 to 65535), E<lt>N2E<gt> is the Information Element ID found in the L<CERT IPFIX Registry|https://tools.netsa.cert.org/cert-ipfix-registry/cert_ipfix_formatted.html> and below, and E<lt>element-ruleE<gt> is a PCRE regular expression and will be stored and associated with the ID number preceding it.  There can be multiple lines for a single application label, however each should have a different E<lt>N2E<gt>.  There should be parentheses around the substring you want to capture and store.  If there is more than 1 set of parentheses in the regular expression, the most outer set is the substring captured.  (See PCRE documentation for details on regular expressions and substring matching.)

=head2 User Defined Elements

To define your own information elements, use the following form:

label E<lt>NE<gt> user E<lt>EE<gt> name E<lt>element-nameE<gt> E<lt>element-ruleE<gt>

where E<lt>NE<gt> is the application label found in yafApplabelRules.conf file. E<lt>EE<gt> is the Information Element ID in the range of 0 to 65535 to be given to the element upon export.  This number should be unique to this file and should NOT be defined in the CERT IPFIX Registry.  This element will be added to the template upon processing of this file, and must be added to the B<yaf> collecting process in order to properly decode the IPFIX message.  E<lt>element-nameE<gt> is the name you want to give to this IPFIX Information Element.  This name can consist of letters and numbers and underscores; it can not contain special characters or spaces.  E<lt>label-ruleE<gt> is the PCRE regular expression and will be stored and associated with the Information Element ID and name preceding it.  There is a limit of 30 additional fields per protocol that YAF will store and execute.  To find out if B<yaf> accepted your elements, run B<yaf> with --verbose.  All user elements will be exported using the CERT Private Enterprise Number (PEN) 6871.  ONLY user labels for protocols FTP, HTTP, IMAP, SMTP, RTSP, SSH, and SIP will be added.  Elements will be added to the template in the order they are
...
limit [field|total] E<lt>limit-valueE<gt>

If the "field" label is present, the E<lt>limit-valueE<gt> will be the number of bytes B<yaf> will export for
...
If the "total" field is present, the E<lt>limit-valueE<gt> will be the total number of bytes B<yaf> may export
...
For more information on B<yaf> information elements see B<yaf(1)>.  For more information on IPFIX Structured lists, see the Internet Draft, Export of Structured Data in IPFIX, E<lt>RFC 6313E<gt>.  Most of the elements are exported as a basicList. An IPFIX basicList represents a list of zero or more instances of any Information Element (IE 291).
...
2821.  B<Note:>  The templates used for SMTP DPI changed in YAF 2.12.0.
...
=item B<smtpHello> CERT (PEN 6871) IE 162, variable length, string

SMTP Hello or Extend Hello command.  Captures the command and the domain name
of the SMTP client.

=item B<smtpEnhanced> CERT (PEN 6871) IE 170, variable length, string

Enhanced SMTP.  Contains the ESMTP command with the following argument.

=item B<smtpMessageSize>, CERT (PEN 6871) IE 330, 4, unsigned

SMTP Message Size

=item B<smtpStartTLS>, CERT (PEN 6871) IE 326, 1, unsigned

START TLS. Element indicating whether or not the SMTP session sent the START
TLS command.

=item B<smtpResponseList>, CERT (PEN 6871) IE 331, variable length, basicList

A basicList of SMTP Replies, represented by smtpResponse (IE CERT/169, variable
length, string) elements.  Each smtpResponse contains of a three digit number
followed by text.

=item B<smtpMessageList> CERT (PEN 6871) IE 336, variable length, subTemplateList

A subTemplateList containing zero (but usually at least one) or more records
described by the yaf_smtp_message template.  Each record represents a single
email message sent during the SMTP conversation.  That template contains the
following elements:

=over 2

=item B<smtpSubject> CERT (PEN 6871) IE 166, variable length, string

The subject of the message.

=item B<smtpToList> CERT (PEN 6871) IE 332, variable length, DPI basicList

The SMTP Recipient (RCPT) Command.  A basicList containing smtpTo (IE CERT/164,
variable length, string) elements.  Each smtpTo captures the command and the
forward-path of the recipient of the mail data.

=item B<smtpFromList> CERT (PEN 6871) IE 333, variable length, DPI basicList
...
=item B<smtpFilenameList> CERT (PEN 6871) IE 334, variable length, DPI basicList
...
=item B<smtpURLList> CERT (PEN 6871) IE 335, variable length, DPI basicList

SMTP URL.  A basicList containing smtpURL (IE CERT/329, variable
length, string) elements.  Each smtpURL contains a URL captured in the SMTP
message body, if any.

=item B<smtpHeaderList> CERT (PEN 6871) IE 337, variable length, DPI subTemplateList

A subTemplateList containing zero or more records described by the
yaf_smtp_header template.  Each record represents a single header (SMTP field
name and body) in the email DATA.  That template contains the following
elements:

=over 2

=item B<smtpKey> CERT (PEN 6871), IE 327, variable length, string

SMTP Header key string.  The name of the header (for example, "To", "From").

=item B<smtpValue> CERT (PEN 6871), IE 328, variable length, string
...
By default, B<yaf> exports 14 information element's in the SSH
...
=item B<sshServerVersion> (PEN 6871) IE 472, variable length, string

The version string from an SSH server.

=item B<sshKeyExchangeAlgorithm> CERT (PEN 6871) IE 476, variable length

The negotiated key exchange algorithm used for an SSH session.

=item B<sshHostKeyAlgorithm> CERT (PEN 6871) IE 477, variable length

The negotiated host key algorithm used for an SSH session.

=item B<sshServerHostKey> CERT (PEN 6871) IE 478, variable length

The MD5 hash of the public key of the SSH server.

=item B<sshCipher> CERT (PEN 6871) IE 473, variable length

The negotiated symmetric encryption algorithm used for an SSH session.

=item B<sshMacAlgorithm> CERT (PEN 6871) IE 474, variable length

The negotiated MAC algorithm used for an SSH session.

=item B<sshCompressionMethod> CERT (PEN 6871) IE 475, variable length

The negotiated compression algorithm used for an SSH session.

=item B<sshHassh> CERT (PEN 6871) IE 468, variable length

The client HASSH MD5 hash of the B<sshHasshAlgorithms> (CERT/469) fingerprint for an SSH client.

=item B<sshServerHassh> CERT (PEN 6871) IE 470, variable length

The server HASSH MD5 hash (hasshServer) of the B<sshServerHasshAlgorithms> (CERT/471) fingerprint for an SSH server.

=item B<sshHasshAlgorithms> CERT (PEN 6871) IE 469, variable length

The SSH client hasshAlgorithms: the concatenated name-lists of the client-to-server algorithms delimited by a semicolon. Element B<sshHassh> (CERT/468) holds the MD5 of this.

=item B<sshServerHasshAlgorithms> CERT (PEN 6871) IE 471, varable length

The SSH server hasshServerAlgoritms: the concatenated name-lists of the server-to-client algorithms delimited by a semicolon. Element B<sshServerHassh> (CERT/470) holds the MD5 of this.
...
=item B<sslClientJA3> CERT (PEN 6871) IE 463, variable length

The JA3 MD5 hash of the sslClientJA3Fingerprint (CERT/464) calculated on the client-side TLS/SSL fingerprint string.

=item B<sslClientJA3Fingerprint> CERT (PEN 6871) IE 464, variable length
...
=item B<sslServerJA3S> CERT (PEN 6871) IE 465, variable length

The JA3S MD5 hash of the sslServerJA3SFingerprint (CERT/466) calculated on the server-side TLS/SSL fingerprint string.

=item B<sslServerJA3SFingerprint> CERT (PEN 6871) IE 466, variable length
...
Emily Sarneso and the CERT Network Situational Awareness Group Engineering Team  E<lt>netsa-help@cert.orgE<gt>, http://www.cert.org/netsa
